...
def p_net(channel):
    ...
    :return: p_net Model
    """
    inputs = ks.Input((None, None, channel))
    ...
    # NOTE: below new layer need added if p_net size = 16
    # x = kl.Conv2D(24, 3, activation=prelu, kernel_regularizer=ks.regularizers.l2(5e-4))(x)
...
def r_net(channel):
    ...
    :return: r_net model
    """
    inputs = ks.Input((r_net_size, r_net_size, channel))
...
def o_net(channel):
    ...
    :return: o_net model
    """
    inputs = ks.Input((o_net_size, o_net_size, channel))
